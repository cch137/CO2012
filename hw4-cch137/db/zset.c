...
static db_bool_t zset_has_member(DBZSet *zset, const char *member)
{
  if (!zset || !member)
    return false;
  DBHashEntry *element = ht_get_entry(zset->dict, member);
...
db_uint_t zadd(DBZSet *zset, db_double_t score, const char *member)
{
  ...
  ht_add_entry(zset->dict, _ht_create_zsetele_entry(duplicated_member, element));
...
DBObj *zscore(DBZSet *zset, const char *member)
{
  ...
  DBHashEntry *entry = ht_get_entry(zset->dict, member);
  if (!entry)
    return dbobj_create_null();
...
db_uint_t zrem(DBZSet *zset, const char *member)
{
  ...
  DBZSetElement *element = _dbobj_extract_zsetele(ht_extract_entry(ht_remove_entry(zset->dict, member)));
